	...
	<div>
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					<el-col :span="12">
						<el-form-item label="投诉编号" prop="tousubianhao">
							<el-input class="list_inp" v-model="form.tousubianhao" :readonly="true" placeholder="投诉编号" />
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="投诉时间" prop="tousushijian">
							<el-date-picker
								class="list_date"
								v-model="form.tousushijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.tousushijian?true:false"
								placeholder="请选择投诉时间" />
						</el-form-item>
					</el-col>
					<el-col :span="12">
					...
					<el-col :span="24">
						<el-form-item label="投诉内容" prop="tousuneirong">
							<editor :value="form.tousuneirong" placeholder="请输入投诉内容" :readonly="!isAdd||disabledForm.tousuneirong?true:false"
								class="list_editor" @change="(e)=>editorChange(e,'tousuneirong')"></editor>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.tousuneirong = res.data.data.tousuneirong?(res.data.data.tousuneirong.replace(reg,'../../../cl84067193/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.tousushijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				if(x=='tousubianhao'){
					form.value.tousubianhao = row[x];
					disabledForm.value.tousubianhao = true;
					continue;
				}
				if(x=='tousuneirong'){
					form.value.tousuneirong = row[x];
					disabledForm.value.tousuneirong = true;
					continue;
				}
				if(x=='tousushijian'){
					form.value.tousushijian = row[x];
					disabledForm.value.tousushijian = true;
					continue;
				}
				if(x=='yonghuzhanghao'){
